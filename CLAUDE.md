...
6. ⏳ Дизайн публичного отчёта
   ...
   - ✅ ИИ-выводы (OpenAI): чекбокс «Сгенерировать с ИИ» в форме создания и редакторе; активен только если блок conclusions включён; gpt-4o-mini генерирует 4–6 абзацев HTML на основе снапшота (трафик, каналы, поисковики, позиции, GSC, Вебмастер); поддерживает сравнительный период; ключ `OPENAI_SECRET_KEY` в `.env.local`
   - ⏳ Мобильная адаптация
